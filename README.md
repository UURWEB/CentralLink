...
- Username-and-password registration and login with no member email required
...
Members do **not** enter an email address. CentralLink converts each username into a private, deterministic internal Firebase login alias. Firebase's Email/Password provider is used only behind the scenes to securely store and verify passwords.
...
Passwords are handled by Firebase Authentication, not stored in Firestore. Members only see and use a username and password; no real email address is collected.

Because members do not provide email addresses, Firebase email password-reset links are not available. A forgotten-password workflow must be handled by a CentralLink administrator.
